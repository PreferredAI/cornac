...
class CrossValidation(EvaluationStrategy):
    ...
    def __init__(self, data,n_folds=5,good_rating = 1.,partition = None, data_train=None, data_validation=None, data_test=None):
    ...
    def get_partition(self):
        ...
        n_fold_partition = np.random.choice(self.n_folds, size=self.data_nnz,replace=True,p=None) #sample with replacement
    ...
    def get_next_train_test_split(self):
        ...
        self.current_split = Split(self.data,good_rating = self.good_rating,index_train = index_train,index_test = index_test)
    ...
    def run_exp(self,model,metrics):
        ...
        for fold in range(self.n_folds):
            ...
            self.get_next_train_test_split()
        ...
        avg_resAvg = resAvg.mean(0)  #we are averaging the average results across the n_folds, another possibility is to make it per-user?
        ...
        resPerU[which_(resPerU[:,len(metrics)].todense().A1,">",0),:] = resPerU[which_(resPerU[:,len(metrics)].todense().A1,">",0),:]/resPerU[which_(resPerU[:,len(metrics)].todense().A1,">",0),len(metrics)].todense().reshape(n_processed_u,1)
