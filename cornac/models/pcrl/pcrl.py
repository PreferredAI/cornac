...
class Pcrl_:
    def __init__(self, cf_data, aux_data, k=100,z_dims = [300], n_epoch=300,batch_size = 300,learning_rate = 0.001, B = 1,w_determinist = True, init_params = None):
        ...
        self.aa = 0.3
        self.bb = 0.3
        self.Ls = sp.csc_matrix((self.aux_data.data.shape[0],self.k))   #Variational Shape parameters of the item factors (Beta in the paper)   
        self.Lr = sp.csc_matrix((self.aux_data.data.shape[0],self.k))   #Variational Rate parameters  of the item factors (Beta in the paper)
    ...
    def dG(self,epsilon, alpha, beta): 
        return ( (alpha - 1./3) * (3./tf.sqrt(9.*alpha - 3.)) * (1.+epsilon/tf.sqrt(9.*alpha-3.))**2 )/beta
    ...
    def pf_(self,X,k, max_iter = 1,init_params = None):
        ...
        res = {'Z':G_s/G_r,'G_s':G_s,'G_r':G_r,'W':L_s/L_r,'Lt':Lt,'Lb':Lb,'Zik':np.array(Zik,dtype='float32'),'Tk':np.array(Tk,dtype='float32')}
    ...
    def generative_net(self,Z,reuse=None):
        ...
        if self.w_determinist:
            ...
            e =  tf.random_normal(tf.shape(self.generator_params[0]), dtype=tf.float32, mean=0., stddev=1.0, name='epsilon')
            h2 = tf.nn.relu(tf.matmul(Z, self.generator_params[0]+0.01*e))
            for l in range (1,self.L):
                e =  tf.random_normal(tf.shape(self.generator_params[l]), dtype=tf.float32, mean=0., stddev=1.0, name='epsilon')
                h2 = tf.nn.relu(tf.sparse_matmul(h2, self.generator_params[l]+0.01*e,a_is_sparse=True)) 
            e =  tf.random_normal(tf.shape(self.generator_params[self.L]), dtype=tf.float32, mean=0., stddev=1.0, name='epsilon')
    ...
    def loss(self,C,X_g,X_,alpha,beta,z,E,Zik,Tk):
        ...
        # KL Divergence term
        kl_term = (alpha-self.aa-Zik)*tf.digamma(alpha) - tf.lgamma(alpha) +  (self.aa+Zik)*tf.log(beta) + alpha*(Tk + self.bb - beta)/beta
    ...
    def learn(self):
        ...
        E = tf.placeholder(tf.float32, shape=[None, self.k],name='E')      #matrix of accepted samples epsilon, epsilon ~ pi(epsilon)
        ...
        for epoch in range(self.n_epoch):
            ...
            resPF = self.pf_(self.cf_data, k= self.k, max_iter = 1, init_params = {'G_s':resPF['G_s'],'G_r':resPF['G_r'],'L_s':self.Ls,'L_r':self.Lr})
        self.Gs = resPF['G_s']
        self.Gr = resPF['G_r']
