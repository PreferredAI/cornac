...
class NeuMFPoiss(Recommender):
    ...
    * He, X., Liao, L., Zhang, H., Nie, L., Hu, X., & Chua, T. S. (2017, April). Neural collaborative filtering. \
    ...
    def _fit_neumf(self):
        ...
        from .ops import gmf, mlp, train_fn
        ...
        with graph.as_default():
            ...
            log_llh = self.labels * tf.log(self.prediction) - self.prediction
            log_llh = tf.reduce_sum(log_llh, axis=1)
            log_llh = tf.reduce_mean(log_llh, axis=0)
        ...
        item_indices = np.arange(self.train_set.num_items)

        loop = trange(self.num_epochs, disable=not self.verbose)
        for _ in loop:
            count = 0
            sum_loss = 0
            for i, batch_users in enumerate(self.train_set.user_iter(self.batch_size)):
                batch_ratings = self.train_set.matrix[batch_users].A
                num_users = len(batch_users)
                batch_users = batch_users.repeat(self.train_set.num_items)

                _, _loss = self.sess.run([train_op, self.loss], feed_dict={
                    self.gmf_user_id: batch_users,
                    self.mlp_user_id: batch_users,
                    self.item_id: np.tile(item_indices, num_users),
