...
def bpr(X, k, lamda = 0.01, n_epochs=100, learning_rate=0.001, batch_size = 100, init_params=None):
    ...
    for epoch in range(n_epochs):
        ...
        Scorei = torch.sum(regU * regI, dim = 1)  
        Scorej = torch.sum(regU * regJ, dim = 1)  

        loss = lamda * (regU_unq.norm().pow(2) + regI_unq.norm().pow(2)) - torch.log(torch.sigmoid(Scorei - Scorej)).sum()
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
