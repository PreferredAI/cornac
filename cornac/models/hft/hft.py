...
class Model:
    ...
    def __init__(self, params, n_user, n_item,dict ,n_vocab, k=10,
        ...
        self.kappa, self.gamma_u, self.gamma_i, self.topic_word = self._get_view(params)  # Parameter views
        # Counter
    ...
    def random_int_topic(self, docs):
        ...
        print(self.background_word)
        print(self.topic_word)
    ...
    def assign_word_topics(self, docs):
        for di in range(len(docs)):
            ...
            for wi in range(doc_len):
                ...
                word = doc[wi]
                topic_score = np.exp(
                    self.kappa * self.gamma_i[di] + self.background_word[word] + self.topic_word[word])
    ...
    def update_params(self, rating_data):
        ...
        self.kappa, self.gamma_u, self.gamma_i, self.topic_word = self._get_view(self.params)

        word_idx = self.topic_word.argsort(axis=0)[-10:, :][::-1]
        # print(word_idx)
    ...
    def _get_view(self, params):
        ...
        params_length = np.array([1, self.n_user * self.k, self.n_item * self.k, self.n_vocab * self.k])
        idx = params_length.cumsum()

        kappa = params[0]
        gamma_u = params[idx[0]:idx[1]].reshape(self.n_user, self.k)
        gamma_i = params[idx[1]:idx[2]].reshape(self.n_item, self.k)
        topic_word = params[idx[2]:idx[3]].reshape(self.n_vocab, self.k)

        return kappa, gamma_u, gamma_i, topic_word
    ...
    def _func(self, X, *args):
        ...
        kappa, gamma_u, gamma_i, topic_word = self._get_view(params=X)
        grad = np.zeros_like(X)
        #dalpha, dkappa, dbeta_u, dbeta_i, dgamma_u, dgamma_i, dtopic_word = self._get_view(params=grad)
        dkappa, dgamma_u, dgamma_i, dtopic_word = self._get_view(params=grad)
        ...
        for i in range(self.n_user):
            ...
            R_i = R_user[i]
            #pred = np.sum(gamma_u[i] * gamma_items, axis=1)
            pred = gamma_items.dot(gamma_u[i])
        ...
        for j in range(self.n_item):
            ...
            pred = gamma_users.dot(gamma_i[j])
        ...
        if self.latent_reg > 0:
            reg_loss += self.latent_reg * np.sum(gamma_u ** 2)
            dgamma_u += 2 * self.latent_reg * gamma_u
            reg_loss += self.latent_reg * np.sum(gamma_i ** 2)
            dgamma_i += 2 * self.latent_reg * gamma_i
