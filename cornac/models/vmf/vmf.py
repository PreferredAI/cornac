...
def vmf(train_set, item_feature, k, d, n_epochs, batch_size, lambda_u, lambda_v,
    ...
    optimizer = torch.optim.RMSprop([U, V], lr= learning_rate)

    for epoch in range(1, n_epochs + 1):
        sum_loss = 0.
        count = 0
        progress_bar = tqdm(total=train_set.num_batches(100),
        ...
        for batch_u, batch_i, batch_r in train_set.uir_iter(batch_size, shuffle=True):
            ...
            Xui = torch.sigmoid(torch.sum(U_u * V_i, dim=1)) 
    ...
    res = {'U':U.data.cpu().numpy(),'V':V.data.cpu().numpy()}
